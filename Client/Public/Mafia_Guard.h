...
class CMafia_Guard : public CMonster
{
protected:
	//Throw_Item, Jump도 추가할 예정.
	enum ANIM_STATE {
		IDLE, WALK, PUNCH, DIVE, THROW_ITEM, SPIN_ATTACK, BLOCKING, FLINCH, DEAD, STAND_SCARED1, RUN_SCARED,
		STAND_SCARED2, DEAD_POP, CHEER1, CHEER2, CHEER3, GUARDIDLE, PLEAD, SLEEP, BULL, ANIM_END
	};
	//enum class ANIM_STATE{IDLE, WALK, SMASH, DIVE, ATTACK, DEAD, HURT, JUMP, JUMP_ATTACK, BLOCK_ATTACK, THROW, STATE_END};
	enum class BEHAVIOR_STATE { IDLE, WALK, ATTACK, HURT, DEAD, BLOCK, GUARD_IDLE, BULLY, STATE_END };
	enum class ATTACK_STATE { PUNCH, DIVE, THROW, STATE_END };
	enum class WEAPON_TYPE { TYPE_NONE, PUNCH, ORANGE, HEAD, TYPE_END };

	/*Type  0: 가드 1: 가는사람  2: 다구리*/
private:
	CMafia_Guard(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CMafia_Guard(const CMafia_Guard& rhs);
	virtual ~CMafia_Guard() = default;
...
protected:
	...
	_bool	Sleep(_float fTimeDelta);
	_bool	Guard_Idle(_float fTimeDelta);
	_bool	Bully(_float fTimeDelta);
	void	Walk_ArrviePoint(_float fTimeDelta);

	void	Init_Type();

public:
	void	Detect_TeamFight();
private:
	void	Create_Ring();
...
public:
	static CMafia_Guard* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
