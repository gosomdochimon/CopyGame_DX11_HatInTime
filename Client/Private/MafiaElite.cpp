...
CMafiaElite::CMafiaElite(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	:CMonster(pDevice, pContext)
{
}

CMafiaElite::CMafiaElite(const CMafiaElite & rhs)
	:CMonster(rhs)
{
}

HRESULT CMafiaElite::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMafiaElite::Initialize(void * pArg)
{
...
void CMafiaElite::Tick(_float fTimeDelta)
{
...
void CMafiaElite::Late_Tick(_float fTimeDelta)
{
	if (m_bDelete)
		return;
...
_float CMafiaElite::Take_Damage(float fDamage, void * DamageType, CGameObject * DamageCauser)
{
	if (fDamage <= 0.f)
		return 0.f;
...
void CMafiaElite::Update_Collider(void)
{
...
void CMafiaElite::Play_FSM(_float fTimeDelta)
{
	if (!m_bIntroEnd)
	{
...
_bool CMafiaElite::Idle(_float fTimeDelta)
{
	...
	else
	{
		m_fIdleTime = 1.f;
		m_eCurrentState = BEHAVIOR_STATE::WALK;
		m_eAttackState = (ATTACK_STATE::PRE_CHARGE);
	}
...
_bool CMafiaElite::Move(_float fTimeDelta)
{
	return _bool();
}

_bool CMafiaElite::Death(_float fTimeDelta)
{
	...
	if (m_bAnimFinished)
	{
		...
		Create_Puff();
		CDeligate_Monster* pDeligate = GET_INSTANCE(CDeligate_Monster);
		pDeligate->Notify_MafiaEliteDead();
		RELEASE_INSTANCE(CDeligate_Monster);
		m_bDelete = true;
	}
...
_bool CMafiaElite::Attack(_float fTimeDelta)
{
	...
	case ATTACK_STATE::DIVE:
		...
		break;
	case ATTACK_STATE::PRE_CHARGE:
		bAttackFinished = Pre_Charge(fTimeDelta);
		break;
	case ATTACK_STATE::CHARGE:
		bAttackFinished = Charge(fTimeDelta);
		break;
...
_bool CMafiaElite::Follow_Enemy(_float fTimeDelta)
{
...
_bool CMafiaElite::Punch(_float fTimeDelta)
{
...
_bool CMafiaElite::Throw_Item(_float fTimeDelta)
{
...
_bool CMafiaElite::Dive_Attack(_float fTimeDelta)
{
...
_bool CMafiaElite::Charge(_float fTimeDelta)
{
	if (m_eCurrentAnimState != ANIM_STATE::CHARGE)
	{
		m_eCurrentAnimState = ANIM_STATE::CHARGE;
		m_bAnimLoop = true;
		m_pModelCom->Set_NextAnimIndex(m_eCurrentAnimState, m_bAnimLoop);
		m_bAnimFinished = false;
		m_fCurrentTime = 0.f;
	...
	m_pTransformCom->Go_Straight(fTimeDelta * 3.f, m_pNavigationCom, 0, &m_bCanGo);

	if (!m_bCanGo)
	{
		if (m_CloneInfo.iTypeNum == 0)
		{//일반 공격
			m_eCurrentState = BEHAVIOR_STATE::HANK;
			m_pEquipments->Change_Part((_uint)WEAPON_TYPE::TYPE_NONE, (_uint)WEAPON_TYPE::TYPE_NONE);
...
_bool CMafiaElite::Pre_Charge(_float fTimeDelta)
{
	if (m_eCurrentAnimState != ANIM_STATE::PRE_CHARGE)
	{
		m_eCurrentAnimState = ANIM_STATE::PRE_CHARGE;
		m_bAnimLoop = false;
		m_pModelCom->Set_NextAnimIndex(m_eCurrentAnimState, m_bAnimLoop);
		m_bAnimFinished = false;
		m_fCurrentTime = 0.f;
...
_bool CMafiaElite::Hank(_float fTimeDelta)
{
	if(m_eCurrentAnimState != ANIM_STATE::HANK)
	{
		m_eCurrentAnimState = ANIM_STATE::HANK;
		m_bAnimLoop = false;
		m_pModelCom->Set_NextAnimIndex(m_eCurrentAnimState, m_bAnimLoop);
		m_bAnimFinished = false;
		m_fCurrentTime = 0.f;
...
_bool CMafiaElite::Hurt(_float fTimeDelta)
{
...
void CMafiaElite::Create_Ring()
{
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Ring"), LEVEL_GAMEPLAY, TEXT("Layer_Effect"), &EffectInfo)))
		return;

	Safe_Release(pGameInstance);
}

void CMafiaElite::Create_HitImpact()
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_HitEffect"), LEVEL_GAMEPLAY, TEXT("Layer_Effect"), &EffectInfo)))
		return;

	Safe_Release(pGameInstance);
}

void CMafiaElite::Create_Puff()
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
void CMafiaElite::Play_Sound(_uint iSoundNum)
{
	if (!m_bSoundPlay)
		return;
...
_bool CMafiaElite::IsDetect(_uint eSphere_Scale, CCollider * pTargetCollider)
{
...
void CMafiaElite::IsShoot_Objects(_bool bShoot)
{
}

HRESULT CMafiaElite::Create_ThrowingObject()
{
	return E_NOTIMPL;
}

void CMafiaElite::Walk_ArrviePoint(_float fTimeDelta)
{
	if (m_CloneInfo.iTypeNum == 0 || m_CloneInfo.iTypeNum == 1)
	{
		if (!m_bArrive)
		{
			if (m_eCurrentAnimState != ANIM_STATE::WALK)
			{
				...
				m_pModelCom->Set_NextAnimIndex(m_eCurrentAnimState);
				m_bAnimFinished = false;
				m_pTransformCom->LookAt(XMVectorSetW(XMLoadFloat3(&m_CloneInfo.fRot_Value), 1.f));
			}

			_float3 Current_Pos;
			XMStoreFloat3(&Current_Pos, m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION));
			if (fabs(Current_Pos.x - m_CloneInfo.fRot_Value.x) <= 0.2f &&
				fabs(Current_Pos.z - m_CloneInfo.fRot_Value.z) <= 0.2f)
			{
				CDeligate_Monster* pDeligate = GET_INSTANCE(CDeligate_Monster);
...
void CMafiaElite::Ready_Detect(_float3 fColliderPos)
{
...
HRESULT CMafiaElite::Ready_Parts()
{
...
HRESULT CMafiaElite::Ready_Components()
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), iLevelIndex, TEXT("Prototype_Component_Model_MafiaElite"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CMafiaElite::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CMafiaElite * CMafiaElite::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMafiaElite*	pInstance = new CMafiaElite(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMafiaElite"));
...
CGameObject * CMafiaElite::Clone(void * pArg)
{
	CMafiaElite*	pInstance = new CMafiaElite(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMafiaElite"));
		Safe_Release(pInstance);
	}
