...
void CParticle_Point::Tick(_float fTimeDelta)
{
	...
	CTrigger_Box* pTarget = dynamic_cast<CTrigger_Box*>(pGameInstance->Get_FirstObject(LEVEL_GAMEPLAY, TEXT("Layer_TriggerBox")));
	if (pTarget != nullptr)
	{
		if (pTarget->Get_GateOpen() == true)
			Set_DeleteObject();
	}
...
HRESULT CParticle_Point::Ready_Components()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_chkpt_stars_fg"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;

	/* For.Com_VIBuffer */
	if (FAILED(__super::Add_Components(TEXT("Com_VIBuffer"), LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Point_Instance_Up"), (CComponent**)&m_pVIBufferCom)))
		return E_FAIL;
