...
HRESULT CUI_Base::SetUp_ShaderResources()
{
	/*if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CUI_Base * CUI_Base::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CUI_Base*	pInstance = new CUI_Base(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CUI_Base"));
...
CGameObject * CUI_Base::Clone(void * pArg)
{
	CUI_Base*	pInstance = new CUI_Base(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CUI_Base"));
		Safe_Release(pInstance);
	}
