#include "stdafx.h"
#include "..\Public\Level_GamePlay2.h"

#include "GameInstance.h"
#include "Camera_Dynamic.h"
#include "DataManager.h"
CLevel_GamePlay2::CLevel_GamePlay2(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CLevel(pDevice, pContext)
{
}

HRESULT CLevel_GamePlay2::Initialize()
{
...
void CLevel_GamePlay2::Tick(_float fTimeDelta)
{
...
void CLevel_GamePlay2::Late_Tick(_float fTimeDelta)
{
	__super::Late_Tick(fTimeDelta);

	SetWindowText(g_hWnd, TEXT("게임플레이2레벨입니다."));
}

HRESULT CLevel_GamePlay2::Ready_Lights()
{
...
HRESULT CLevel_GamePlay2::Ready_Layer_Player(const _tchar * pLayerTag)
{
...
HRESULT CLevel_GamePlay2::Ready_Layer_Monster(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_GamePlay2::Ready_Layer_Map(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_GamePlay2::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_GamePlay2::Ready_Layer_Effect(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_GamePlay2::Ready_Layer_Camera(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_GamePlay2::Ready_Layer_UI(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_GamePlay2::Ready_Layer_InteractionObject(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Throne"), LEVEL_GAMEPLAY, TEXT("Layer_TriggerBox"))))
		return E_FAIL;
	Safe_Release(pGameInstance);
...
CLevel_GamePlay2 * CLevel_GamePlay2::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLevel_GamePlay2*	pInstance = new CLevel_GamePlay2(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		ERR_MSG(TEXT("Failed to Created : CLevel_GamePlay2"));
