...
HRESULT CCell::Initialize(const _float3 * pPoints, _uint iEnum, _int iIndex)
{
	m_iIndex = iIndex;
...
CCell * CCell::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const _float3 * pPoints, _uint iEnum, _int  iIndex)
{
	CCell*	pInstance = new CCell(pDevice, pContext);

	if (FAILED(pInstance->Initialize(pPoints, iEnum, iIndex)))
	{
